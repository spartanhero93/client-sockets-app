...
import { generateRandomChildren } from '../helpers/generateRand'
...
class NewFactory extends Component {
  ...
  handleLowerBound = event => {
    ...
    if (val < 0 || val > this.state.upperBound) {
      return this.setState({ isLowerBoundValid: false })
    }
    this.setState({ lowerBound: val, isLowerBoundValid: true })
  }

  handleUpperBound = event => {
    const val = event.target.value

    if (val > 10000 || val <= this.state.lowerBound) {
      return this.setState({ isUpperBoundValid: false })
    }
    this.setState({ upperBound: val, isUpperBoundValid: true })
  }
